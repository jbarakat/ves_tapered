...
for redvol in REDVOL :
	...
	#	DP = DPE
		...
		xL = CA
		if (STR == 'DP') :
			yL = DP*pow(delta,0.5)/(4.0*sqrt(2.0)*pi)
		#	for i in range(len(DP)) :
		#		yL[i] = DP[i]*pow(EPS[i],0.5)/(4.0*sqrt(2.0)*pi)
		if (STR == 'EPS') :
			yL = EPS
		if (STR == 'ETA') :
			yL = ETA
		...
		xi  = np.logspace(-6, floor(log10(max(xL))), 10000)
	...
	#	plt.plot(xL,yL,'-',c='k')
		plt.plot(xL,yL,'.',c=c)
	#	plt.plot(xi,yi,'.',c='k', label = '$\lambda/\lambda_c$ = ' + str(lam))
			
		if (confin == CONFIN[0] or confin == CONFIN[-1]) :
			plt.text(xi[-1], yi[-1], str(lam),fontsize = 10)
...
plt.xlabel('$\mathrm{Ca} = \mu R^2 U / \kappa$',fontsize=fs)
if (STR == 'DP') :
